...
> {
  try {
    ...
    let take = limit;
    if (keyset) {
      // created_at es NOT NULL en MovimientoCaja: un cursor con `c: null` no
      // puede provenir de una página real de este listado → cursor inválido.
      // La guarda también estrecha keyset.createdAt a Date para la condición
