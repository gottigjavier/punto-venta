...
> {
  try {
    ...
    const whereComun = {
      ...
      ...(fecha_desde || fecha_hasta ? { created_at: rango } : {}),
    };
    ...
    const filas: FilaHistorial[] = [
      ...
      ...movimientos.map((m) => ({
        ...
        estado: (m.tipo === "ingreso" ? "Ingreso" : "Egreso") as
          | "Ingreso"
          | "Egreso",
        cantidad_items: null,
        referencia_id: null,
    ...
    const filtradas = tipo_fila
      ? filas.filter((f) => f.tipo_fila === tipo_fila)
      : filas;
    ...
    return err(
      databaseError("Error al obtener historial unificado", error as Error),
    );
  }
}
