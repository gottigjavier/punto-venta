...
// Get rubro by ID
export async function getRubroById(id: string): Promise<AppResult<Rubro>> {
  try {
...
export async function updateRubro(
  ...
  try {
    ...
    const updateData: Prisma.RubroUncheckedUpdateInput = {};
    ...
    logger.info(
      { rubroId: rubro.id, nombre: rubro.nombre },
      "Rubro actualizado",
    );
    return ok(rubro as Rubro);
  } catch (error) {
