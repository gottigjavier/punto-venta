...
describe("Stock/lote Use Cases (modelo Lote)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Las operaciones blindadas corren dentro de prisma.$transaction(cb). El mock
    // ejecuta el callback con el propio mockPrisma como client de transacción: los
    // tests configuran tx.$queryRaw / tx.lote.* / tx.producto.* vía mockPrisma.*
  ...
  describe("loteIngreso", () => {
    ...
    it("merge: misma (producto_id, numero_lote, fecha_vencimiento) suma cantidad y promeda precio (106.67)", async () => {
      ...
      // $queryRaw returns array of merged lotes
      mockPrisma.$queryRaw.mockResolvedValue([existing]);
    ...
    it("misma numero_lote distinto vencimiento crea segundo lote (no modifica existente)", async () => {
      ...
      // $queryRaw con venc nuevo → null (no existe lote con ese venc)
      mockPrisma.producto.findUnique.mockResolvedValue(producto);
    ...
    it("numero_lote null crea lote nuevo sin buscar merge", async () => {
      ...
      // null nunca va a $queryRaw
      mockPrisma.lote.create.mockResolvedValue(
        mockLoteRaw({ id: "lote-null", numero_lote: null }),
      );
  ...
  describe("loteList", () => {
    ...
    function mockFindManyVigentes(
      vigentes: Record<string, unknown>[],
      todos: Record<string, unknown>[],
    ) {
      mockPrisma.lote.findMany.mockImplementation(
        (opts: { select?: unknown; include?: unknown }) => {
          // query de sumas: select con cantidad_disponible
          if (opts?.select) {
            return Promise.resolve(
              vigentes.map((l) => ({
                producto_id: (l as { producto_id: string }).producto_id,
                cantidad_disponible: (l as { cantidad_disponible: number })
                  .cantidad_disponible,
              })),
            );
          }
        ...
        (opts: { where?: any; select?: unknown }) => {
          ...
          if (estadoFilter?.estado === "activo") {
            filtered = todos.filter(
              (l) => (l as { estado: string }).estado === "activo",
            );
          } else if (
            estadoFilter?.estado &&
            (estadoFilter.estado as { in?: unknown }).in
          ) {
            const inArr = (estadoFilter.estado as { in: string[] }).in;
            filtered = todos.filter((l) =>
              inArr.includes((l as { estado: string }).estado),
            );
          }
    ...
    it("retorna fila POR LOTE; default solo activo; stock_bajo usa SUM de lotes activos no vencidos", async () => {
      ...
      const lote1 = mockLoteRaw({
        id: "l1",
        numero_lote: "L1",
        cantidad_disponible: 3,
        cantidad_aviso: 5,
      });
      const lote2 = mockLoteRaw({
        id: "l2",
        numero_lote: "L2",
      ...
      const result = await loteList({
        ...STOCK_QUERY_BASE,
        archivados: undefined,
      });
      ...
      const listCall = mockPrisma.lote.findMany.mock.calls.find(
        (c) => !c[0]?.select,
      );
    ...
    it("badge por_vencer default 30; 45 días→ok; venc NULL→ok", async () => {
      ...
      const byId = r1.value.data.reduce(
        (acc, row) => {
          acc[row.id] = row;
          return acc;
        },
    ...
    it("badge por_vencer usa preaviso del producto (custom 15 días)", async () => {
      ...
      const byId = result.value.data.reduce((acc, row) => {
        acc[row.id] = row;
        return acc;
      }, {} as any);
    ...
    it("archivados=true devuelve SOLO lotes terminales (RF-02)", async () => {
      ...
      expect(listCall?.[0]?.where?.AND).toContainEqual({
        estado: { in: ["agotado", "vencido", "descartado"] },
      });
      if (result.isOk()) {
  ...
  describe("loteEdit", () => {
    it("NO toca cantidad_disponible al editar", async () => {
      ...
      // FOR UPDATE: el código ahora lee el lote con $queryRaw (no findUnique)
      mockPrisma.$queryRaw.mockResolvedValue([
        {
          id: lote.id,
    ...
    it("CONFLICT si duplica (numero_lote, fecha_vencimiento) de otro lote", async () => {
      ...
      mockPrisma.lote.findFirst.mockResolvedValue({
        ...mockLoteRaw(),
        id: "otro-lote",
      });
    ...
    it("NOT_FOUND si el lote no existe", async () => {
      mockPrisma.$queryRaw.mockResolvedValue([]);
      const result = await loteEdit("non-existent", {
        numero_lote: null,
        precio_compra: 200,
      });
      expect(result.isErr()).toBe(true);
  ...
  describe("loteRetirar", () => {
    it("marca descartado desde activo", async () => {
      ...
      mockPrisma.$queryRaw.mockResolvedValue([
        { id: lote.id, producto_id: lote.producto_id, estado: lote.estado },
      ]);
  ...
  describe("loteDelete", () => {
    it("CONFLICT si tiene DetalleVenta asociado", async () => {
      const lote = mockLoteRaw();
      mockPrisma.$queryRaw.mockResolvedValue([
        { id: lote.id, producto_id: lote.producto_id },
      ]);
    ...
    it("borra físicamente si NO tiene DetalleVenta", async () => {
      ...
      expect(mockPrisma.lote.delete).toHaveBeenCalledWith({
        where: { id: "lote-1" },
      });
    });
