...
function buildTx(txOverrides: Record<string, unknown> = {}): {
  tx: unknown;
...
describe("Venta Use Cases (modelo Lote)", () => {
  ...
  describe("createVenta (FEFO sobre lotes)", () => {
    ...
    it(
      ...
      withFakeNow(async () => {
        ...
        const L4 = mockLoteDb({
          ...
          fecha_vencimiento: new Date("2026-09-15T00:00:00.000Z"),
          created_at: new Date("2026-01-02"),
        });
        const { tx, spies } = buildTx();
        spies.loteQueryRaw.mockResolvedValue([L1Agotado, L4]);
        spies.loteUpdate.mockResolvedValue({});
        spies.detalleVentaCreate.mockResolvedValue({});
        spies.ventaCreate.mockResolvedValue({
          id: "venta-1",
    ...
    it(
      ...
      withFakeNow(async () => {
        ...
        spies.loteQueryRaw.mockResolvedValue([L5, L6]);
        spies.loteUpdate.mockResolvedValue({});
    ...
    it(
      ...
      withFakeNow(async () => {
        ...
        const { tx, spies } = buildTx();
        spies.loteQueryRaw.mockResolvedValue([LxBajo]);
        mockPrisma.$transaction.mockImplementation(
          async (fn: (tx: unknown) => Promise<unknown>) => fn(tx),
        );
        ...
        if (result.isErr()) {
          ...
          if (result.error.code === "STOCK_INSUFFICIENT") {
    ...
    it(
      ...
      withFakeNow(async () => {
        ...
        const { tx, spies } = buildTx();
        spies.loteQueryRaw.mockResolvedValue([loteVencido, loteActivo]);
        spies.loteUpdate.mockResolvedValue({});
        spies.detalleVentaCreate.mockResolvedValue({});
        spies.ventaCreate.mockResolvedValue({
          id: "venta-1",
    ...
    it(
      ...
      withFakeNow(async () => {
        ...
        const { tx, spies } = buildTx();
        spies.loteQueryRaw.mockResolvedValue([LExacto]);
  ...
  describe("deleteVenta", () => {
    ...
    it(
      "revierte stock por lote: agotado→activo; vencido si venc<hoy",
      withFakeNow(async () => {
        const loteVigente = mockLoteDb({
          id: "LV",
          cantidad_disponible: 0,
          estado: "agotado",
          fecha_vencimiento: new Date("2026-12-01T00:00:00.000Z"),
        ...
        mockPrisma.$transaction.mockImplementation(
          async (fn: (tx: unknown) => Promise<unknown>) =>
            fn({
              // Foto FOR UPDATE: deleteVenta re-valida cierre_caja_id/estado
              // dentro de la tx (anti-TOCTOU con cerrarCaja). Venta abierta → pasa.
  ...
  describe("cerrarCaja", () => {
    ...
    it("CONFLICT si NO hay ventas abiertas (foto FOR UPDATE vacía)", async () => {
      const { verifyPassword } = await import(
        "../../../infrastructure/auth/password.js"
      );
      mockPrisma.usuario.findUnique.mockResolvedValue({
      ...
      vi.mocked(verifyPassword).mockResolvedValue(true);
      // La foto estable de ventas abiertas se lee con FOR UPDATE dentro de la
      // tx (flujo blindado). Vacía → NO_OPEN_SALES → CONFLICT.
