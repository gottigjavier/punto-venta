...
async function main(): Promise<void> {
  // SE1: el seed es exclusivo de desarrollo / entornos de prueba. En producción
  // el primer administrador se crea con el asistente de setup (POST
  // /auth/bootstrap), nunca con credenciales por defecto. Abortar aquí también
  // sirve de red de seguridad si alguien corre `prisma db seed` por accidente
  // en un entorno productivo.
  if (process.env["NODE_ENV"] === "production") {
    console.log(
      "⚠️  Seed omitido: no se ejecuta en producción (usar /auth/bootstrap)",
    );
    return;
  }
